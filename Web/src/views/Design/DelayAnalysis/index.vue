...
export default defineComponent({
  ...
  setup() {
    ...
    function getRatio(
      ...
      let S = 0;
      let Q = 0;
      rf.canalize_info[roadIndex].road_sign.enter.map((r: any, j: number) => {
        //j是真实的路标所在位置
        if (road_key.indexOf(r.key) > -1) {
          S = rf.flow_info.saturation[roadIndex][j].number;
          Q += S * λ;
        }
      });
